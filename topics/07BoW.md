...
It can be observed that the negative of the log-likelihood is accumulating the cross-entropy for all the elements in the dataset $$\mathcal D$$, i.e., probability $$p(k)$$ is $$\mathbb 1(y=k)$$, and $$q(k)=f_k(x)$$ using the previous definition of cross-entropy $$H(p, q)$$. The characteristic note is that $$y$$ and $$x$$ are constants in the inner summation, and variable $$k$$ goes from all the classes. Therefore, minimizing the log-likelihood is minimizing the cross-entropy, which acts as the loss function $$L$$ in the [optimization problem.](/NLP-Course/topics/02Vocabulary/#eq:supervised-learning-optimization) 
